...
function App() {
  ...
  const [currentHexagon, setCurrentHexagon] = useState({col: -300, row : -1000})
  ...
  const mapCoordToIndex = [[null,null,16, 17 , 18], [null,12,13,14,15], [7,8,9,10,11], [3,4,5,6,null], [0,1,2,null,null]];
  ...
  function drawHexCoordinates(can, center, hexagon, index){
    ...
    ctx.fillText(mapCoordToIndex[hexagon.row][hexagon.col], center.x, center.y + 8)
  ...
  function drawHexes(){
    ...
    for (let q = -radius; q <= radius; q++) {
      ...
      for (let r = r1; r <= r2; r++) {
        ...
        list.push({index: mapCoordToIndex[r + radius][q + radius], row: r + radius, col : q + radius, center: center})
        fillHex(canvasId,center,"#c9c6cc")
        drawHex(canvasId, {x: center.x, y: center.y})
        drawHexCoordinates(canvasId, center, Hex(q + radius, r + radius), counter)
      }
    }
  ...
  function drawHex(can, center, color, width){
    for(let i = 0; i < 6; i++){
      ...
      drawLine(can,{x: start.x, y : start.y}, {x: end.x, y : end.y}, color, width)
  ...
  function hexToPixel(hex){
    let x = hexSize * (Math.sqrt(3) * hex.col  +  Math.sqrt(3)/2 * hex.row) + hexOrigin.x
  ...
  switch(edge){
   ...
   drawLine(canvasId,{x: c0.x + x0Factor, y : c0.y + y0Factor}, {x: c1.x + x1Factor, y : c1.y + y1Factor}, color, 10)
 ...
 function handleClick(e){
  ...
  function handleMove (e){
    ...
    const {col,row, s} = hex_round(pixel_to_pointy_hex(Point(offsetX, offsetY)))
  ...
  return (
    <>
    <div className="App" style={{position : "relative", display: "block", height: 650}}>
      <canvas height={600} width={600} ref={canvasId} ></canvas>
      <canvas height={600} width={600} ref={canvasCoords} onMouseMove={handleMove} onClick={handleClick}></canvas>
    </div>
    <div style={{position : "relative", display: "block"}}>
      <input type="number" onChange={(e)=>{setRoad(parseInt(e.target.value) % 6)}}/>
      <button onClick={()=>{
        // drawRoad(0,1,"navy");
